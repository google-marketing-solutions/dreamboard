...
export class SceneBuilderComponent implements OnInit {
  ...
  refreshStory(): void {
    // TODO(nsampsell): Get the user ID from a shared service.
    const userId = '123';
    this.storiesStorageService.getStoryById(userId, this.story.id).subscribe({
      next: (story: VideoStory) => {
        this.story = story;
        this.componentsCommunicationService.updateScenes(this.story.scenes);
      },
      error: (error: any) => {
        console.error(error);
        openSnackBar(this._snackBar, `Error refreshing story: ${error.error.detail}`, 5);
      }
    });
  }
}